...
const Typing: React.FC<TypingProps> = ({
    ...
    const renderIncorrectWhitespace = (index: number, ch: string) => (
        ...
        >
            ...
            {hintModeToUse && (
                <span
                    className={`absolute text-accent-foreground/30 left-1/2 -translate-x-1/2 top-1/2 ${wrongTextClass[textSizeToUse]}`}
                >
    ...
    const renderIncorrectChunk = (start: number, end: number) => {
        ...
        for (const wsIndex of whiteSpaceIndex) {
            if (segmentStart <= wsIndex - 1) {
                content.push(
                    <span
                        key={`seg-${segmentStart}-${wsIndex - 1}`}
                        className="relative"
                    >
                        <span className={getClassName(0)}>
                            {renderSymbol(displayedText.slice(segmentStart, wsIndex))}
                        </span>
                        {hintModeToUse && (
                            <span
                                className={`absolute text-accent-foreground/30 left-1/2 -translate-x-1/2 top-1/2 ${wrongTextClass[textSizeToUse]}`}
                            >
                                {renderSymbol(userInput.slice(segmentStart, wsIndex))}
                            </span>
                        )}
                    </span>
                )
        ...
        if (segmentStart <= end) {
            content.push(
                <span
                    key={`seg-${segmentStart}-${end}`}
                    className="relative"
                >
                    <span className={getClassName(0)}>
                        {renderSymbol(displayedText.slice(segmentStart, end + 1))}
                    </span>
                    {hintModeToUse && (
                        <span
                            className={`absolute text-accent-foreground/30 left-1/2 -translate-x-1/2 top-1/2 ${wrongTextClass[textSizeToUse]}`}
                        >
                            {renderSymbol(userInput.slice(segmentStart, end + 1))}
                        </span>
                    )}
                </span>
            )
        ...
        for (const wsIndex of whiteSpaceIndex) {
            if (segmentStart <= wsIndex - 1) {
                content.push(
                    <span
                        key={`seg-${segmentStart}-${wsIndex - 1}`}
                        className={getClassName(booleanChar)}
                    >
                        {renderSymbol(displayedText.slice(segmentStart, wsIndex))}
                    </span>
                )
            ...
            content.push(
                <span
                    ...
                    className={getClassName(booleanChar)}
                >
                    {renderSymbol(displayedText[wsIndex])}
                </span>
            );
            if (displayedText[wsIndex] === "\n") {
                content.push(<br key={`br-${wsIndex}`} />);
